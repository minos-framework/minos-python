...
class TestPostgreSqlSnapshotWriter(PostgresAsyncTestCase):
    ...
    async def test_dispatch_ignore_previous_version(self):
        ...
        async with PostgreSqlRepository.from_config(self.config) as r:
            with patch("minos.common.PostgreSqlRepository.select", _fn):
                async with PostgreSqlSnapshotWriter.from_config(self.config, repository=r) as dispatcher:
                    await dispatcher.dispatch()
            async with PostgreSqlSnapshotReader.from_config(self.config, repository=r) as snapshot:
