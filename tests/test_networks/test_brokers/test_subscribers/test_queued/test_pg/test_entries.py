...
class TestHandlerEntry(unittest.IsolatedAsyncioTestCase):
    ...
    def test_constructor(self):
        entry = BrokerHandlerEntry(1, "AddOrder", 0, self.data_bytes, 1)
    ...
    def test_constructor_extended(self):
        ...
        entry = BrokerHandlerEntry(1, "AddOrder", 0, self.data_bytes, 1, created_at=now, updated_at=now)
        self.assertEqual(1, entry.id)
        self.assertEqual("AddOrder", entry.topic)
        self.assertEqual(0, entry.partition)
        self.assertEqual(self.data_bytes, entry.data_bytes)
    ...
    def test_sort(self):
        unsorted = [
            BrokerHandlerEntry(1, "", 0, FakeModel("foo").avro_bytes, 1),
            BrokerHandlerEntry(2, "", 0, FakeModel(4).avro_bytes, 1),
            BrokerHandlerEntry(3, "", 0, FakeModel(2).avro_bytes, 1),
            BrokerHandlerEntry(4, "", 0, FakeModel(3).avro_bytes, 1),
            BrokerHandlerEntry(5, "", 0, FakeModel(1).avro_bytes, 1),
            BrokerHandlerEntry(6, "", 0, FakeModel("bar").avro_bytes, 1),
        ]
    ...
    def test_eq(self):
        now = current_datetime()
        one = BrokerHandlerEntry(1, "AddOrder", 0, self.data_bytes, 1, created_at=now, updated_at=now)
        two = BrokerHandlerEntry(1, "AddOrder", 0, self.data_bytes, 1, created_at=now, updated_at=now)
