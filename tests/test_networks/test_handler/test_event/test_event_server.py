...
class TestEventServer(PostgresAsyncTestCase):
    ...
    def test_from_config(self):
        dispatcher = EventHandlerServer.from_config(config=self.config)
        self.assertIsInstance(dispatcher, EventHandlerServer)

    def test_from_config_raises(self):
        with self.assertRaises(MinosConfigException):
            EventHandlerServer.from_config()
    ...
    async def test_queue_add(self):
        ...
        event_server = EventHandlerServer.from_config(config=self.config)
