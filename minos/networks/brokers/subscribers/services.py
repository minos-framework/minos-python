...
class BrokerSubscriberService(Service):
    """Broker Producer Service class."""

    @inject
    def __init__(self, impl: BrokerSubscriber = Provide["broker_subscriber"], **kwargs):
        super().__init__(**kwargs)

        if impl is None or isinstance(impl, Provide):
            raise NotProvidedException(f"A {BrokerSubscriber!r} object must be provided.")
    ...
    async def start(self) -> None:
        ...
        if isinstance(self.impl, QueuedBrokerSubscriber):
            await self.impl.run()
