...
class AvroDataDecoder:
    ...
    def _cast_model_type(self, type_: ModelType, data: Any) -> Any:
        ...
        if isinstance(data, dict):
            with suppress(Exception):
                decoded_data = {n: self._cast_value(t, data.get(n, None)) for n, t in type_.type_hints.items()}
        ...
        with suppress(Exception):
            ...
            decoded_data = (self._cast_value(t, d) for d, t in zip(decoded_data, type_.type_hints.values()))
            return type_(*decoded_data, additional_type_hints=type_.type_hints)
